...
export default function Agentes() {
  ...
  const globalKPIs = agentes?.reduce(
    (acc, agent) => ({
      ...
      avgOnTime: (acc.avgOnTime * acc.count + (agent.stats?.on_time_pct || 0)) / (acc.count + 1),
      avgNPS: (acc.avgNPS * acc.count + (agent.stats?.nps_avg || 0)) / (acc.count + 1),
      count: acc.count + 1,
    }),
    { activeAgents: 0, totalBookings: 0, totalM3: 0, totalMargin: 0, avgOnTime: 0, avgNPS: 0, count: 0 }
  ) || { activeAgents: 0, totalBookings: 0, totalM3: 0, totalMargin: 0, avgOnTime: 0, avgNPS: 0, count: 0 };
  ...
  return (
    <div className="container-dashboard space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        <AgentKPICard title="Agentes Activos" value={globalKPIs.activeAgents} icon={Users} />
        <AgentKPICard title="Mudanzas con Agentes" value={globalKPIs.totalBookings} icon={Package} />
        <AgentKPICard title="Volumen Total (m³)" value={globalKPIs.totalM3.toFixed(1)} icon={TrendingUp} />
        <AgentKPICard title="Margen Total" value={`$${globalKPIs.totalMargin.toLocaleString()}`} icon={DollarSign} />
      </div>

      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        <AgentKPICard title="On-time % Global" value={`${globalKPIs.avgOnTime.toFixed(1)}%`} icon={AlertCircle} />
        <AgentKPICard title="NPS Promedio" value={globalKPIs.avgNPS.toFixed(1)} icon={ThumbsUp} />
        <AgentKPICard title="Total Agentes" value={agentes?.length || 0} icon={Users} />
      </div>
      ...
      <div className="space-y-4">
        {filteredAgentes && filteredAgentes.length > 0 ? (
          filteredAgentes.map((agente) => <AgentCard key={agente.id} agent={agente} />)
        ) : (
          <div className="text-center py-12">
            <Users className="w-12 h-12 mx-auto mb-4 text-muted-foreground" />
            <h3 className="text-lg font-semibold mb-2">No se encontraron agentes</h3>
            <p className="text-muted-foreground mb-4">Ajusta los filtros o agrega nuevos agentes a tu red</p>
            <Button>
              <Plus className="w-4 h-4 mr-2" />
              Agregar Agente
            </Button>
